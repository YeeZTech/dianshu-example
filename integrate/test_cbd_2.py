...
def get_input_param():
    demo1_name = "CBD-demo1"
    demo1_key = demo1_name + ".key.json"
    demo1_result = demo1_name + ".summary.json"
    result_file = demo1_name + ".result.json"
    param = {
        "use-privatekey-file": demo1_key,
        "encrypted-result-file": demo1_result,
        "package-type": "company",
        "output": result_file
    }
    common.decrypt_result(**param)
    with open(result_file, 'r') as of:
        return json.load(of)['result']


if __name__ == "__main__":
    name = "CBD-demo2"
    crypto = "stdeth"
    data = os.path.join(common.example_dir, "./dataset/行业字典.csv")
    parser = os.path.join(common.example_lib,
                          "company_statistics_parser.signed.so")
    plugin = os.path.join(common.example_lib, "libindustry_reader.so")

    input_param = get_input_param()
