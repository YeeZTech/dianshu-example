...
dianshu_parser::dianshu_parser(const input_param_t &param) : m_param(param) {
  size_t s = ypc::simple_sealed_file::blockfile_t::BlockSizeLimit;
  m_mem_buf.reset(new char[s]);
  m_mem_buf_size = s;
}

dianshu_parser::~dianshu_parser() = default;
...
uint32_t dianshu_parser::parse() {
  ...
  m_parser =
      std::make_shared<dianshu_parser_module>(parser_enclave_path.c_str());
