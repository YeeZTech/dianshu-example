...
def generate_top_cmake(target_dir, name):
    ...
    with open(fp, 'w') as cm:
        ...
        cm.write("include_directories(${YPC_INSTALL_DIR}/include)\n")
        cm.write("link_directories(${YPC_INSTALL_DIR}/lib)\n")
        cm.write(
...
def generate_reader_cmake(reader_dir, name, config):
    ...
    with open(fp, 'w') as cm:
        ...
        cm.write("target_link_libraries({}_csv_reader pthread ff_net)\n".format(name))
        cm.write("install(TARGETS {}_csv_reader DESTINATION lib)".format(name))


def generate_csv_reader_file(cpp, name):
    cpp.write("#include<toolkit/plugins/csv/csv_reader.h>\n")
    cpp.write(
        "typedef ypc::plugins::typed_csv_reader<{}_item_t> {}_reader_t;\n".format(name, name))
...
def generate_mysql_reader_file(cpp, name, config):
    ...
    cpp.write(
        "constexpr static const char * table_name = \"{}\";".format(config["table_name"]))
    cpp.write("};\n")
    cpp.write(
        "typedef ff::sql::table<ff::sql::mysql<ff::sql::cppconn>, _mysql_table_t")
    ...
    cpp.write(
        "typedef ypc::plugins::typed_mysql_reader<{}_item_t, _table_t> {}_reader_t;\n".format(name, name))
        ...
        for item in schema:
            ...
            if config["type"] == "mysql":
                udth.write("define_column({}, {}, {}, \"{}\");\n".format(
                    item["name"], item["col"], item["type"], item["colname"]))
            ft = ft + item["name"] + ","
        ft = ft.strip(',')
        ft = ft + '> {}_item_t;\n'.format(name)
        udth.write(ft)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Generate plugins for support data sources. A part of Fidelius.')
