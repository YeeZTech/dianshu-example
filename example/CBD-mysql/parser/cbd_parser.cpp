...
cbd_parser::cbd_parser(const input_param_t &param) : m_param(param) {}

cbd_parser::~cbd_parser() = default;
...
uint32_t cbd_parser::parse() {
  ...
  m_parser = std::make_shared<cbd_parser_module>(parser_enclave_path.c_str());
#ifdef DEBUG
...
uint32_t cbd_parser::dump_result(const ypc::bytes &res) {
...
uint32_t cbd_parser::feed_datasource() {
...
uint32_t cbd_parser::feed_model() {
...
uint32_t cbd_parser::feed_param() { return ypc::success; }

uint32_t cbd_parser::next_data_batch(const uint8_t *data_hash,
...
void cbd_parser::free_data_batch(uint8_t *data) { delete[] data; }
