...
public:
  ...
  inline stbox::bytes do_parse(const stbox::bytes &param) {
    ...
    for ( auto it : mo.values() ) {
      ...
      std::vector<std::string> fields = parseCSVLine(row); // 逗号分隔的数据存入fields
    ...
    for ( auto it : mo.values() ) {
      ...
      std::vector<bool> rowVecBool(column, false); // 记录缺失值情况，默认为缺失(false)
      int ti = 0; // 记录循环中的列数
    ...
    pack_j["NullProportion"] = nullProportion.substr(0, nullProportion.length() - 4);
